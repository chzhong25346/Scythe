...
class PsacSpider(scrapy.Spider):
    ...
    # def parse_url(self, response):
        ...
        wti_wcs_diff = get_cell(response, "Diff (WTI-WCS)")
        sp_500_energy = get_cell(response, "S&P 500 Energy", cell_num=0)
        amex_oil = get_cell(response, "AMEX Oil Index", cell_num=0)
        tsx_energy = get_cell(response, "S&P/TSX Capped Energy", cell_num=0)
        tsx_og_producers = get_cell(response, "S&P/TSX O&G Producers", cell_num=0)
        tsx_og_services = get_cell(response, "S&P/TSX O&G Services", cell_num=0)
        ca_us_ex_rate = get_cell(response, "Can-US Exch. Rate (US ¢)", cell_num=0)
